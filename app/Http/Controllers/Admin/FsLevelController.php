...
class FsLevelController extends AdminBaseController
{
    public function index(Request $request)
    {
        ...
        return view('admin.fs_level.index', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (FsLevel::where('level', $data['level'])->first())
            return responseWrong('已存在该等级的返水等级');
    ...
    public function update(Request $request, $id)
    {
        ...
        if (FsLevel::where('level', $data['level'])->where('id', '!=', $id)->first())
            return responseWrong('已存在该等级的返水等级');
